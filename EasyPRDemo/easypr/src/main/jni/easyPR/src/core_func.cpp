...
#define VERTICAL 0
    ...
    float computeIOU2(const Rect& rect1, const Rect& rect2) {
        
        Rect inter = interRect(rect1, rect2);
        Rect urect = mergeRect(rect1, rect2);
        
        if (rect2.x == inter.x && rect2.y == inter.y && rect2.width == inter.width && rect2.height == inter.height) {
            return 1;
        }
        else{
            float iou = (float)inter.area() / (float)urect.area();
    ...
    void mserCharMatch(const Mat &src, std::vector<Mat> &match, std::vector<CPlate>& out_plateVec_blue, std::vector<CPlate>& out_plateVec_yellow,
        ...
        const double maxAreaRatio = 1;
        
        Ptr<MSER2> mser;
        mser = MSER2::create(delta, minArea, int(maxAreaRatio * imageArea));
        //mser detect,  mser-,  mser+,	comment by liangdas
...
#pragma omp parallel for
        for (int color_index = 0; color_index < 2; color_index++) {
            ...
            for (size_t index = 0; index < size; index++) {
                ...
                if (verifyCharSizes(rect)) {
                    ...
                    // remove the small lines in character like "zh-cuan", "川"
            ...
            NMStoCharacter(charVec, overlapThresh);//use nms to remove reduction box, comment by liangdas
            charVec.shrink_to_fit();
            
            /////////////////////////for debug///////////////////
#if 0
            for (auto charCandidate : charVec) {
                cv::rectangle(result, charCandidate.getCharacterPos(), Scalar(0, 255, 0), 1);//draw rectangle on result img(result img is a gray)
            }
            if (1) {
                std::stringstream ss(std::stringstream::in | std::stringstream::out);
                ss << "resources/image/tmp/" << img_index << "_1_" << "searcCharVec.jpg";
			...
			////////////////////for debug //////////////////////
			std::vector<CCharacter> strongWeekSeedVec;
			//////////////////////////////////////////////
            for (auto charCandidate : charVec) {
				...
				if (charCandidate.getVecforGroup()){
					strongWeekSeedVec.push_back(charCandidate);
				}
			...
			//NMStoCharacter(strongWeekSeedVec, overlapThresh);

            // merge chars to group
            std::vector<std::vector<CCharacter>> charGroupVec;
            charGroupVec.reserve(64);
			//mergeCharToGroup(strongSeedVec, charGroupVec);
			mergeCharToGroup(strongWeekSeedVec, charGroupVec);
            ...
            for (auto charGroup : charGroupVec) {
                ...
                for (auto character : roCharGroup) {
                    ...
                    cv::rectangle(result, charRect, Scalar(0, 255, 0), 1);//draw rectangle on result img(result img is a gray)
            ...
            for (auto plate : plateVec) {
                ...
                //////////////////////////////////
                bool ret = mserCharacter[0].getIsChinese();
                //if (mserCharacter.size() < char_max_count) {  //comment by liangdas, for all the
                if (!ret)
                {
                    if (1 && showDebug) {
                ...
                if (verifyRotatedPlateSizes(platePos)) {
                    rotatedRectangle(result, platePos, Scalar(0, 0, 255), 1);//comment by liangdas, only draw the plate rect on image
                    ...
                    if (1 && showDebug) {
                        std::stringstream ss(std::stringstream::in | std::stringstream::out);
                        ss << "resources/image/tmp/" << img_index << "_1_" << "plate_rotate.jpg";
                        imwrite(ss.str(), result);
                    }
            ...
            if (1) {
                std::stringstream ss(std::stringstream::in | std::stringstream::out);
                ss << "resources/image/tmp/plate_" << img_index << "_" << the_color << ".jpg";
                imwrite(ss.str(), result);
            }
        }
    }
    ...
    //  计算一个矩形的外包矩形，input： roi_rect, src, 返回结果是：safeBoundRect
    bool calcSafeRect(const RotatedRect &roi_rect, const Mat &src,
        ...
        //  a new rect not out the range of matidi

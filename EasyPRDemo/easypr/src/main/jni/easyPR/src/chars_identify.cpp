...
namespace easypr {
    ...
    CharsIdentify::CharsIdentify() {
        ...
        kv_->load(kEtcProvince);
    }
    
    /////////////////  add by liangmin /////////////////////
    CharsIdentify* CharsIdentify::instance(std::string ann_path,std::string ann_chinese_path,std::string etc_path) {
        if (!instance_) {
            instance_ = new CharsIdentify(ann_path, ann_chinese_path, etc_path);
        }
        return instance_;
    }
    
    CharsIdentify::CharsIdentify(std::string ann_path, std::string ann_chinese_path, std::string etc_path) {
        ann_ = ml::ANN_MLP::load<ml::ANN_MLP>(ann_path);
        annChinese_ = ml::ANN_MLP::load<ml::ANN_MLP>(ann_chinese_path);
        kv_ = std::shared_ptr<Kv>(new Kv);
        //kv_->load("etc/province_mapping");
        kv_->load(etc_path);
    }
    ...
    /////////////////add by liangmin///////////////////
    CharsIdentify* CharsIdentify::getInstance(){
        return CharsIdentify::instance_;
    }
    
    void CharsIdentify::releaseModel(){
        if (!ann_->empty())
            ann_->clear();
        if (!annChinese_->empty())
            annChinese_->clear();

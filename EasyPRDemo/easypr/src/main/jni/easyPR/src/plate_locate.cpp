...
namespace easypr {
	...
	void deleteNotArea(Mat &inmat, Color color = UNKNOWN) {
		...
		if (0){
			imshow("deleteNotArea-1", tmpMat);
			waitKey(0);
			destroyWindow("deleteNotArea-1");
		}
		Color plateType;
		if (UNKNOWN == color) {
		...
		if (0) {
			imshow("deleteNotArea-2", inmat);
			waitKey(0);
			destroyWindow("deleteNotArea-2");
		}

		if (bFindLeftRightBound1(img_threshold, posLeft, posRight)) {
			inmat = inmat(Rect(posLeft, top, w - posLeft, bottom - top));
			if (0) {
				imshow("deleteNotArea-3", inmat);
				waitKey(0);
				destroyWindow("deleteNotArea-3");
			}
		}
	}

	int CPlateLocate::deskew_Color(const Mat &src, const Mat &src_b,
		...
		for (size_t i = 0; i < inRects.size(); i++) {
			...
			////////////line fit add by liangmin //////////////
			///////////////////////////////////////////////////
			Point2f rect_points[4];
			roi_rect.points(rect_points);
			std::vector<Point> points,pointsTemp;
			Vec4f line;
			pointsTemp.push_back(rect_points[1]);
			//////////////////// for debug ///////////////////
			if (0){
				src.copyTo(mat_debug);
				cv::circle(mat_debug, pointsTemp[0], 3, Scalar(0, 0, 255), 2);
				imshow("ColorMatch-Point1", mat_debug);
				waitKey(0);
				destroyWindow("ColorMatch-Point1");
			}
			////////////////////////////////////////////////////

			int tmpWidth0 = sqrt((rect_points[1].x - rect_points[0].x) * (rect_points[1].x - rect_points[0].x) + (rect_points[1].y - rect_points[0].y) * (rect_points[1].y - rect_points[0].y));
			int tmpWidth2 = sqrt((rect_points[1].x - rect_points[2].x) * (rect_points[1].x - rect_points[2].x) + (rect_points[1].y - rect_points[2].y) * (rect_points[1].y - rect_points[2].y));
			int abs0 = abs(tmpWidth0 - roi_rect_size.width);
			int abs2 = abs(tmpWidth2 - roi_rect_size.width);
			if (abs0 < abs2) {
				pointsTemp.push_back(rect_points[0]);
				pointsTemp.push_back(rect_points[2]);
				pointsTemp.push_back(rect_points[3]);
			}
			else{
				pointsTemp.push_back(rect_points[2]);
				pointsTemp.push_back(rect_points[0]);
			...
			Point2f pt;
			pt.x = (pointsTemp[0].x + pointsTemp[2].x) / 2;
			pt.y = (pointsTemp[0].y + pointsTemp[2].y) / 2;
			points.push_back(pt);
			pt.x = (pointsTemp[1].x + pointsTemp[3].x) / 2;
			pt.y = (pointsTemp[1].y + pointsTemp[3].y) / 2;
			points.push_back(pt);
			...
			if (0){
				src.copyTo(mat_debug);
				/*cv::circle(mat_debug, rect_points[0], 3, Scalar(0, 255, 0), 2);*/
				cv::circle(mat_debug, pointsTemp[0], 3, Scalar(0, 0, 255), 2);
				cv::circle(mat_debug, pointsTemp[1], 3, Scalar(0, 0, 255), 2);
				cv::circle(mat_debug, pointsTemp[2], 3, Scalar(0, 255, 0), 2);
				cv::circle(mat_debug, pointsTemp[3], 3, Scalar(0, 255, 0), 2);
				imshow("ColorMatch-Drew", mat_debug);
				waitKey(0);
				destroyWindow("ColorMatch-Drew");
			}
			////////////////////////////////////////////////////
			...
			// m_angle=60
  			if (roi_angle - m_angle < 0 && roi_angle + m_angle > 0) {
				...
				//useDeteleArea = 1;
				if (useDeteleArea)
					deleteNotArea(deskew_mat, UNKNOWN);


				if (deskew_mat.cols * 1.0 / deskew_mat.rows > 1.5 &&
					...
					CPlate plate;
					////// change to the right angle   revised liangmin  //////////
					roi_rect.angle = angle;
					roi_rect.size.width = roi_rect_size.width;
					roi_rect.size.height = roi_rect_size.height;
	...
	bool CPlateLocate::rotation(Mat &in, Mat &out, const Size rect_size,
		...
		addWeighted(imageRoi, 0, in, 1, 0, imageRoi);  //计算方法dst(I)=src1(I)*alpha+src2(I)*beta+gamma

		Point2f center_diff(in.cols / 2.f, in.rows / 2.f);
		Point2f new_center(in_large.cols / 2.f, in_large.rows / 2.f);

		Mat rot_mat = getRotationMatrix2D(new_center, angle, 1);//获得一个旋转参数矩阵

		/*imshow("in_copy", in_large);
		waitKey(0);*/

		Mat mat_rotated;
		//旋转结果放在mat_rotated中，原图像是in_large，
...
#pragma omp section
	{
		...
		deskew_Color(src, src_b_blue, rects_color_blue, plates_blue, true, BLUE);
	}
#pragma omp section
	{
		colorSearch(src_clone, YELLOW, src_b_yellow, rects_color_yellow);
		deskew_Color(src_clone, src_b_yellow, rects_color_yellow, plates_yellow, true, YELLOW);
	}
		}
	...
	int CPlateLocate::plateMserLocate(Mat src, vector<CPlate> &candPlates, int img_index) {
		...
		for (size_t i = 0; i < channelImages.size(); ++i) {
			...
			for (size_t j = 0; j < flags.size(); j++) {
				...
				for (auto plate : plates) {
					...
					RotatedRect scaleRect = scaleBackRRect(rrect, (float)scale_ratio);//以前是在缩放后的图计算的车牌大小，把车牌的大小，放缩到原始图的大小
				...
				Mat resize_src_b;
				//comment by liangdas
				//src_b image, binary image,  which has plate on it, plate pixel is 255
				//resize_src_b: binary image, dest image, where store the dest image after resize
				//channelImage is the orignal gray image
                
				resize(src_b, resize_src_b, Size(channelImage.cols, channelImage.rows));
			
				//comment by liangdas
				// src image is the orignal RGB image
				// resize_src_b: binary image, which has plate on it, plate pixel is 255
				//这个函数是把有些变形和旋转的车牌，旋转回来，用到了原始图，二值化图，车牌区域大小，结果保留在deskewPlate中
				...
				for (auto dplate : deskewPlate) {
					...
					for (auto splate : mserPlate) {
						...
						float iou = 0.f;
						//判断矫正前和矫正之后的两个矩形是不是交叠很大，是的话，才认为是车牌
						bool isSimilar = computeIOU(drect, srect, src.cols, src.rows, 0.95f, iou);
						//float score1 = PlateJudge::instance()->plateSetScore(dplate);//judge by svm
						//add by liangdas
						if (0) {
							n++;
							std::stringstream ss(std::stringstream::in | std::stringstream::out);
							ss << "resources/image/tmp/" << "plate_" << img_index << "_" << n << ".jpg";
							imwrite(ss.str(), dmat);
						}
						//float score2 = PlateJudge::instance()->plateSetScore(splate);//judge by svm
						if (isSimilar) {
		...
		if (0) {
			std::stringstream ss(std::stringstream::in | std::stringstream::out);
			ss << "resources/image/tmp/plate_" << "result"<< ".jpg";
			imwrite(ss.str(), src);
		}

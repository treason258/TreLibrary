...
void getHOGFeatures(const Mat& image, Mat& features) {
	...
	HOGDescriptor *hog = new HOGDescriptor(cvSize(136, 36), cvSize(68, 12), cvSize(34, 12), cvSize(34, 12), 9);    
	std::vector<float>descriptors;//结果数组       
	hog->compute(image, descriptors, Size(4, 4)); //调用计算函数开始计算

	int nDim = descriptors.size();
	features = Mat::zeros(1, nDim, CV_32FC1);
	for (int i = 0; i < nDim; i++)
		features.at<float>(i) = descriptors[i];
}

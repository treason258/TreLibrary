...
public class CanvasView extends View {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        // 获取屏幕宽高
        mScreenWidth = Caches.get().getScreenWidth();
        mScreenHeight = Caches.get().getScreenHeight();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int ScrollHeight = mScreenHeight * 5;
        // 绘制竖线
        for (int i = 0; i < mScreenWidth / 100 + 1; i++) {
            canvas.drawLine(i * 100, 0, i * 100, ScrollHeight, mPaint);
        }
        // 绘制横线
        for (int i = 0; i < ScrollHeight / 100 + 1; i++) {
        ...
        mPaint.setTextSize(30);
        mPaint.setColor(Color.RED);
        canvas.drawText("模糊效果：", 100, 1800, mPaint);
        mPaint.setAntiAlias(true);          // 抗锯齿
        mPaint.setColor(Color.RED);         // 画笔颜色
        mPaint.setStyle(Paint.Style.FILL);  // 画笔风格
        mPaint.setTextSize(40);             // 绘制文字大小，单位px
        mPaint.setStrokeWidth(5);           // 画笔粗细
        mBlurMaskFilter = new BlurMaskFilter(3f, BlurMaskFilter.Blur.NORMAL);
        mPaint.setMaskFilter(mBlurMaskFilter);
        canvas.drawText("明月把拥有变作失去-NORMAL", 300, 1800, mPaint);
        mBlurMaskFilter = new BlurMaskFilter(3f, BlurMaskFilter.Blur.OUTER);
        mPaint.setMaskFilter(mBlurMaskFilter);
        canvas.drawText("明月把拥有变作失去-OUTER", 300, 1900, mPaint);
        mBlurMaskFilter = new BlurMaskFilter(3f, BlurMaskFilter.Blur.INNER);
        mPaint.setMaskFilter(mBlurMaskFilter);
        canvas.drawText("明月把拥有变作失去-INNER", 300, 2000, mPaint);
        mBlurMaskFilter = new BlurMaskFilter(3f, BlurMaskFilter.Blur.SOLID);
        mPaint.setMaskFilter(mBlurMaskFilter);
        canvas.drawText("明月把拥有变作失去-SOLID", 300, 2100, mPaint);

        // 浮雕效果 Y=2200
        mPaint.reset();
        mPaint.setTextSize(30);
        mPaint.setColor(Color.RED);
        canvas.drawText("浮雕效果：", 100, 2200, mPaint);
        ...
        mPaint.setStyle(Paint.Style.FILL);  //画笔风格
        mPaint.setTextSize(40);             //绘制文字大小，单位px
        mPaint.setStrokeWidth(8);           //画笔粗细
        mPaint.setMaskFilter(mEmbossMaskFilter);
        canvas.drawText("明月把拥有变作失去", 300, 2200, mPaint);
        canvas.drawBitmap(mBitmap, 300, 2300, mPaint);
